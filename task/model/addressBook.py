    ...
    def load_from_file(self, path):
        with open(path, "r") as file:
            for contactline in file.readlines():
                ...
                phones.pop(0)
                bday = phones.pop(0).strip()
                if any(bday):
                    rec.add_birthday(datetime.fromisoformat(bday))
                for phone in phones:
    ...
    def save_to_file(self, path):
        with open(path, "w") as file:
            for record in self.data:
                file.write(
                    f"{record.name.value};{'' if record.birthday == None else record.birthday.value.isoformat() };{'; '.join(p.value for p in record.phones)}\n"
                )
